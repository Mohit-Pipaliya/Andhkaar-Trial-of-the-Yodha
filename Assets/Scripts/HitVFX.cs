...
public static class HitVFX
{
    ...
    {
        string poolKey = "HitSparks";
        GameObject sparkObj = null;
        
        if (ObjectPoolManager.Instance != null)
        {
            sparkObj = ObjectPoolManager.Instance.SpawnProcedural(poolKey, position);
        }

        if (sparkObj == null)
        {
            sparkObj = new GameObject(poolKey);
            ...
            if (ObjectPoolManager.Instance != null)
            {
                ObjectPoolManager.Instance.RegisterProceduralObject(sparkObj, poolKey);
            }
        }
        else
        {
            sparkObj.transform.position = position;
            ParticleSystem ps = sparkObj.GetComponent<ParticleSystem>();
            if (ps != null)
            {
        ...
        // Particle khatam hone ke baad object ko pool me waapas daal do (Despawn) badle me lag kam hoga
        if (ObjectPoolManager.Instance != null)
        {
            ObjectPoolManager.Instance.Despawn(sparkObj, 1f);
        }
        else
        {
